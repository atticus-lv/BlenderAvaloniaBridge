using Avalonia;
using BlenderAvaloniaBridge.Cef.Sample;
using BlenderAvaloniaBridge.Cef.Sample.Services;

namespace BlenderAvaloniaBridge.Cef;
...
internal static class Program
{
    public static async Task<int> Main(string[] args)
    {
        ...
        try
        {
            if (launch.IsBridgeMode)
            {
                ...
                CefRuntimeBootstrap.Initialize();
                await BlenderBridgeLauncher.RunBridgeAsync(
                    launch,
                    createBridgeWindow: () => new BlenderAvaloniaBridge.Cef.Sample.Views.MainWindow());
                return 0;
            }
    ...
    public static AppBuilder BuildAvaloniaApp()
    {
        return AppBuilder.Configure<App>()
            .UsePlatformDetect()
            .AfterSetup(_ => CefRuntimeBootstrap.Initialize())
            .LogToTrace();
    }
}
