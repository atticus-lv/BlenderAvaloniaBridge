...
public sealed class InputDispatcher
{
    ...
    public InputDispatcher(MainViewModel viewModel)
    {
    ...
    public Task DispatchAsync(Window window, ProtocolEnvelope envelope)
    {
        ...
        {
            case "pointer_move":
                _viewModel.SetStatus($"MouseMove {(int)point.X},{(int)point.Y}");
                window.MouseMove(point, modifiers);
                break;
            case "pointer_down":
                _leftButtonDown = string.Equals(envelope.Button, "left", StringComparison.OrdinalIgnoreCase);
                _viewModel.SetStatus($"PointerDown {envelope.Button ?? "unknown"} {(int)point.X},{(int)point.Y}");
                window.MouseDown(point, ToMouseButton(envelope.Button), modifiers);
                break;
            case "pointer_up":
                _leftButtonDown = false;
                _viewModel.SetStatus($"PointerUp {envelope.Button ?? "unknown"} {(int)point.X},{(int)point.Y}");
                window.MouseUp(point, ToMouseButton(envelope.Button), modifiers);
                break;
            case "wheel":
                _viewModel.SetStatus($"Wheel {envelope.DeltaX ?? 0:0.##},{envelope.DeltaY ?? 0:0.##}");
                window.MouseWheel(point, new Vector(envelope.DeltaX ?? 0, envelope.DeltaY ?? 0), modifiers);
                break;
            case "key_down":
                {
                    var key = KeyMap.ToKey(envelope.Key);
                    _viewModel.SetStatus($"KeyDown {envelope.Key ?? "unknown"}");
            ...
            case "key_up":
                {
                    var key = KeyMap.ToKey(envelope.Key);
                    _viewModel.SetStatus($"KeyUp {envelope.Key ?? "unknown"}");
                    window.KeyRelease(key, modifiers, KeyMap.ToPhysicalKey(envelope.Key), envelope.Text);
                    break;
                }
            case "text":
                _viewModel.SetStatus($"Typed: {envelope.Text ?? string.Empty}");
                window.KeyTextInput(envelope.Text ?? string.Empty);
                break;
            case "focus":
                _viewModel.SetStatus(envelope.Focus == true ? "Focus gained" : "Focus lost");
