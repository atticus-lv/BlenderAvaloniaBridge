...
3. Adjust `Display Size`, `Render Scaling`, and `Target FPS` if needed. `Display Size` is the logical Avalonia size; `Render Scaling` controls render density and the Blender overlay display scale; `Target FPS` defaults to `120`.
